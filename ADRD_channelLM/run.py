...
def train(logger, model, inputs, batch_size, output_dir,
    ...
    optimizer, scheduler = get_optimizer_and_scheduler(
        "adamw",
        model, learning_rate, warmup_steps, num_training_steps
    )
    dataloader = get_dataloader(inputs, batch_size, is_training=True)
    # if the param is not freezed
    ...
    for epoch in range(num_training_steps):
        for batch in dataloader:
            ...
            if len(batch) == 3:
                ...
                labels = batch[3].cuda()
            loss = run_model(model, input_ids,
            ...
            if global_step % eval_period == 0:
                if prompt_tune:
                    ...
                    model_state_dict = {key: model.state_dict()[key if n_gpus == 1 else "module." + key].cpu() for key
                                        in keys}
                else:
...
def run_model(model, input_ids, attention_mask, token_type_ids,
    ...
    """Thi place is the model part!!!"""
    outputs = model(input_ids=input_ids, attention_mask=attention_mask)
    logits = outputs.logits[..., :-1,:].contiguous() # different version [last_hidden_state,hidden_states,attentions]
    print(logits.size())
    # contiguous Returns a contiguous in memory tensor containing the same dat
    if return_logits:
    ...
    if labels is None:
        labels = input_ids
    # TODO why here use the 1 before? because to ignore the [CLS]
    labels = labels[..., 1:].contiguous() #  get input ids except the input
    label_mask = token_type_ids[..., 1:].contiguous()

    loss_fct = torch.nn.CrossEntropyLoss(reduction="none")
    losses = loss_fct(logits.view(-1, logits.size(-1)), # 1D Tensor assigning weight to each of the classes
