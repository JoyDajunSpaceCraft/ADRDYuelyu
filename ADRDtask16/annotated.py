...
with open("/Users/yuelyu/PycharmProjects/ADRD/ADRDtask16/refine_middle_1760_.json","r") as f:
    json_ = json.load(f)
...
for key, values in json_.items():
    ...
    if type(values["valid_context"]) == str:
        for sent in sent_tokenize(values["valid_context"]):
           res.append(sent)
    else:
